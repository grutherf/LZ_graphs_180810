...
#define HEIGHT 59.5 //cm
#define RADIUS 24.3
#define TrFOIL 0.//66802
double pmtRad[2] = {2.540,2.265}; //outer then inner
double gridsA[5] = { 0.05, 0.05, 0.05, 0.20, 0.20 };
double gridsZ[5] = { 1.96, 5.59, 53.9, 54.9, 58.5 };
double gridsD[5] = { 206., 206., 101.6,28.4, 50.8 }; //um
double gridsS[5] = { 1.00, 0.50, 0.50, .025, 0.50 };
double gridsT[5];
#define BORDER 54.4
...
double rand_gauss(); double QuantumEfficiency ( int array, double angle ) {
  ...
  if ( array == 0 ) return (0.256+rand_gauss()*0.03)*(1.-exp(-1.7/cos(angle)))/(1-exp(-1.7));
               else return (0.264+rand_gauss()*0.04)*(1.-exp(-1.7/cos(angle)))/(1-exp(-1.7));
...
int main ( int argc, char** argv ) {
  ...
  gridsT[0] = 1. - (gridsD[0] * 1e-4) / gridsS[0]; //gridsT[0] *= 1. - (7./3.) * ( (gridsD[0] * 1e-4) / gridsS[0] );
  gridsT[1] = 1. - (gridsD[1] * 1e-4) / gridsS[1]; //gridsT[1] *= 1. - (7./3.) * ( (gridsD[1] * 1e-4) / gridsS[1] );
  gridsT[2] = 1. - (gridsD[2] * 1e-4) / gridsS[2]; //gridsT[2] *= 1. - (7./3.) * ( (gridsD[2] * 1e-4) / gridsS[2] );
  gridsT[3] = 1. - (2*gridsD[3]*1e-4) / gridsS[3]; //gridsT[3] *= 1. - (7./3.) * ( (gridsD[3] * 1e-4) / gridsS[3] );
  gridsT[4] = 1. - (gridsD[4] * 1e-4) / gridsS[4]; //gridsT[4] *= 1. - (7./3.) * ( (gridsD[4] * 1e-4) / gridsS[4] );
  ...
  for ( long i = 0; i < numPhotons; i++ ) { firstTime = true; //cout << i << endl;
    ...
    phi = 2.*M_PI*rand_uniform(); r = RADIUS * sqrt ( rand_uniform() );
    direction[0] = r * cos(phi);
    direction[1] = r * sin(phi);
    direction[2] = atof(argv[1]);//HEIGHT * rand_uniform();
  ...
  DO_OVER:
    ...
    case 1:
      if ( ((direction[2] > gridsZ[0] && direction[5] < gridsZ[0]) || (direction[2] < gridsZ[0] && direction[5] > gridsZ[0])) && rand_uniform() > gridsT[0]*cos(kludge*alpha/4.) ) {
	goto BOT;
      }
      if ( arrow == up ) { k = 2; goto DO_OVER; } else { k = 0; goto DO_OVER; }
    case 2:
      if ( ((direction[2] > gridsZ[1] && direction[5] < gridsZ[1]) || (direction[2] < gridsZ[1] && direction[5] > gridsZ[1])) && rand_uniform() > gridsT[1]*cos(kludge*alpha/4.) ) {
	goto CTH;
      }
      if ( arrow == up ) { k = 3; goto DO_OVER; } else { k = 1; goto DO_OVER; }
    case 3:
      if ( ((direction[2] > gridsZ[2] && direction[5] < gridsZ[2]) || (direction[2] < gridsZ[2] && direction[5] > gridsZ[2])) && rand_uniform() > gridsT[2]*cos(kludge*alpha/4.) ) {
	goto GAT;
      }
    ...
    case 6:
      if ( ((direction[2] > gridsZ[4] && direction[5] < gridsZ[4]) || (direction[2] < gridsZ[4] && direction[5] > gridsZ[4])) && rand_uniform() > gridsT[4]*cos(alpha/4.) ) {
	goto TOP;
      }
  ...
  PMTt:
	...
	if ( rand_uniform() < QuantumEfficiency(0,alpha) ) numSurvivors[0]++; else numAbsorbed[1]++;
      ...
      if( rand_uniform() < reflectance(GXe) ) {
	direction[5] -= TrFOIL;
